...
public class Universe : MonoBehaviour
{
    ...
    [Header("UI - Zones Table (TMP)")]
    [Tooltip("Assign the TextMeshPro text object that will display the dynamic zones table.")]
    public TMP_Text zonesTableText;
    [Tooltip("How often (seconds) to refresh the zones table text.")]
    [Range(0.02f, 5f)]
    ...
    void Start()
    {
        ...
        // Initial UI draw (if assigned)
        if (zonesTableText != null)
        {
            zonesTableText.enableWordWrapping = false;
            zonesTableText.richText = true;
            zonesTableText.alignment = TextAlignmentOptions.TopLeft;
            zonesTableText.text = BuildZonesTable();
            _uiRefreshTimer = uiRefreshInterval;
        }
    }
    ...
    void Update()
    {
        ...
        // ---- UI refresh for the dynamic zones table ----
        if (zonesTableText != null)
        {
    ...
    /// <summary>
    /// Builds a dynamic table using TMP's <pos=> tags so columns line up regardless of font.
    /// Widened the gap between Temp and pH to prevent overlap on narrow rects / larger fonts.
    /// </summary>
    private string BuildZonesTable()
    {
        if (zones == null || zones.Length == 0) return "No zones defined.";

        var sb = new StringBuilder(256);
        sb.AppendLine("<b>Environment Zones</b>");

        // Header row (percent columns) — widened pH start and shifted UV/Toxin accordingly.
        // Tweak these if your TMP rect is unusually narrow.
        sb.Append("<pos=0%><b>Zone</b>");
        sb.Append("<pos=22%><b>Temp(°C)</b>");
        sb.Append("<pos=58%><b>pH</b>");     // was 48%, moved to 58% to open space
        sb.Append("<pos=76%><b>UV</b>");     // shifted right
        sb.Append("<pos=90%><b>Toxin</b>\n"); // shifted right, still within bounds

        // Rows: zones displayed as 1–8
        for (int i = 0; i < zones.Length; i++)
        {
            var z = zones[i];
            sb.Append("<pos=0%>").Append((i + 1).ToString("0", CultureInfo.InvariantCulture));
            sb.Append("<pos=22%>").Append(z.temperature.ToString("F1", CultureInfo.InvariantCulture));
            sb.Append("<pos=58%>").Append(z.pH.ToString("F1", CultureInfo.InvariantCulture));
            sb.Append("<pos=76%>").Append(z.uvLightIntensity.ToString("F2", CultureInfo.InvariantCulture));
            sb.Append("<pos=90%>").Append(z.toxinFieldStrength.ToString("F2", CultureInfo.InvariantCulture)).Append('\n');
        }
